          ...
          <div className="flex px-4 py-6 sm:px-0 justify-between gap-2 bg-gray-100">
            <div>
              {notes.map(n => <NoteCard key={n.id} note={n}
                                        onDelete={(id) => notesClient.default.deleteNote(id).then(() => notesClient.default.getNotes()).then(setNotes)}
                                        onEdit={() => null} />)}
            </div>
            ...
            <CreateNote onCreate={(message) => notesClient.default.createNote({message}).then(() => notesClient.default.getNotes()).then(setNotes)}/>
            </div>
          </div>
        </div>
      </main>
